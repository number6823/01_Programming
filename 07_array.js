...
arr[2] = "juice"       // 새로운 값을 추가하는 것 맞는데, 이 방식은 "갑의 수정"을 할 때만 이욜
console.log(arr);


arr.push("cola","water");     // push() 메소드는, 배열의 마지막에 값을 추가할 수 있음. 매개변수에 1개 이상 쓸 수 있음
                              // push()를 실행ㅎ한 결과값 (리턴값)은 해당 배열의 길이(length)가 반환
console.log(arr);


arr.pop();                    // pop() 메소드는 매개변수가 없음
                             // 배열의 마지각에  있는 요소를 빼는기능. 실행 결과값 (리턴값) 은 해당 요소의 값을 반환
console.log(arr);

// 만약, 마지막이 아니라 중간에 값을 추가하고 싶으면? 아니멵 중간에 값을 빼고 싶으면?
...
// splice(1,4,"Cherry", "Mango","Apple") => 인덱스 1 부터 4개의 요소를 삭제하고, 그 자리에 3가지를 넣게ㅛ다
// splice(3,2)    => 인덱스 3부터 2개의 요소를 삭제
// splice(2,0,"Cherry") => 인덱스 2부터,0 개의 요소를 삭제하고, Cherry를 넣겠다
// splice(0,0,"Cherry") => 인데스 0부터,0개의 요소를 삭제하고,Cherry를 넣겠다

// splice(arr.length - 1,0,"Cherry")   => 마지막 자리에,삭제는 안하고, "Cherty"를 넣겠다
